def sonarChecks() {
   stage('Sonar Checks') {
        ...
        sh "sonar-scanner -Dsonar.host.url=http://172.31.92.95:9000 -Dsonar.login=${SONAR_USR} -Dsonar.password=${SONAR_PSW} -Dsonar.projectKey=${COMPONENT} ${ARGS}"
        sh "bash -x sonar-quality-gate.sh ${SONAR_USR} ${SONAR_PSW} 172.31.92.95 ${COMPONENT}" 
